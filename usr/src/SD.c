...
static uint8_t SDCARD_ReadR1() {
    ...
    uint8_t tx = 0xFF;
    for(;;) {
        HAL_SPI_TransmitReceive(&hspi6, &tx, &r1, sizeof(r1), HAL_MAX_DELAY);
...
static int SDCARD_WaitDataToken(uint8_t token) {
    ...
    uint8_t tx = 0xFF;
    for(;;) {
        HAL_SPI_TransmitReceive(&hspi6, &tx, &fb, sizeof(fb), HAL_MAX_DELAY);
        if(fb == token)
            break;
...
static int SDCARD_ReadBytes(uint8_t* buff, size_t buff_size) {
    ...
    uint8_t tx = 0xFF;
    while(buff_size > 0) {
        HAL_SPI_TransmitReceive(&hspi6, &tx, buff, 1, HAL_MAX_DELAY);
        buff++;
        buff_size--;
    }
...
int UL_SD_Init() {
    ...
    */
    ...
    for(int i = 0; i < 10; i++) { // 80 clock pulses
        HAL_SPI_Transmit(&hspi6, &high, sizeof(high), HAL_MAX_DELAY);
    }
    ...
    */
...

    {
        ...
        HAL_SPI_Transmit(&hspi6, (uint8_t*)cmd, sizeof(cmd), HAL_MAX_DELAY);
    }
...
int UL_SD_WriteSingleBlock(uint32_t blockNum, const uint8_t* buff) {
    UL_SD_Select();
    ...
    HAL_SPI_Transmit(&hspi6, &dataToken, sizeof(dataToken), HAL_MAX_DELAY);
    HAL_SPI_Transmit(&hspi6, (uint8_t*)buff, 512, HAL_MAX_DELAY);
    HAL_SPI_Transmit(&hspi6, crc, sizeof(crc), HAL_MAX_DELAY);
...
int UL_SD_ReadData(uint8_t* buff) {
    uint8_t crc[2];
    UL_SD_Select();
...
int UL_SD_WriteData(const uint8_t* buff) {
    UL_SD_Select();
...
int UL_SD_WriteEnd() {
    ...
    HAL_SPI_Transmit(&hspi6, &stopTran, sizeof(stopTran), HAL_MAX_DELAY);
