...
void UL_flight_control_pid_controller(UL_flight_control_typedef* flight_control){
    ...
    flight_control->delta_error[PID_YAW] = RC.diffCapture[3] - flight_control->previous_error[PID_YAW];
    flight_control->delta_error[PID_PITCH] = RC.diffCapture[2] - flight_control->previous_error[PID_PITCH];
    flight_control->delta_error[PID_ROLL] = RC.diffCapture[1] - flight_control->previous_error[PID_ROLL];
    ...
    UL_PWM_SetUs(&brushless[0], RC.diffCapture[0] + flight_control->pid[PID_ROLL] + flight_control->pid[PID_PITCH] - flight_control->pid[PID_YAW]);
    UL_PWM_SetUs(&brushless[1], RC.diffCapture[0] - flight_control->pid[PID_ROLL] + flight_control->pid[PID_PITCH] + flight_control->pid[PID_YAW]);
    UL_PWM_SetUs(&brushless[2], RC.diffCapture[0] + flight_control->pid[PID_ROLL] - flight_control->pid[PID_PITCH] + flight_control->pid[PID_YAW]);
    UL_PWM_SetUs(&brushless[3], RC.diffCapture[0] - flight_control->pid[PID_ROLL] - flight_control->pid[PID_PITCH] - flight_control->pid[PID_YAW]);*/
...
void UL_flight_control_print(UL_flight_control_typedef* flight_control){
    ...
    UL_TFT_ST7735_WriteNumber(&Tft, 25, 20, RC.diffCapture[3]/10, FONT_7x10, ST7735_WHITE, ST7735_BLACK);
    UL_TFT_ST7735_WriteNumber(&Tft, 50, 20, RC.diffCapture[2]/10, FONT_7x10, ST7735_WHITE, ST7735_BLACK);
    UL_TFT_ST7735_WriteNumber(&Tft, 75, 20, RC.diffCapture[1]/10, FONT_7x10, ST7735_WHITE, ST7735_BLACK);
    UL_TFT_ST7735_WriteNumber(&Tft, 100, 20, RC.diffCapture[0]/10, FONT_7x10, ST7735_WHITE, ST7735_BLACK);
